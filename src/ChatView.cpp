...
public:
    ...
    virtual void Draw(BRect updateRect) {
        // Rendering is done in ChatView's Draw method
    }
...
void HtmlContainer::draw_background(litehtml::uint_ptr hdc, const std::vector<litehtml::background_paint>& bg)
{
    ...
    for (auto it = bg.rbegin(); it != bg.rend(); ++it) {
        ...
        if (!bgPaint.image.empty() && bgPaint.image_size.width > 0 && bgPaint.image_size.height > 0) {
            // In a real implementation, you would draw the background image here
            // For this simple version, we skip image drawing
        }
    }
